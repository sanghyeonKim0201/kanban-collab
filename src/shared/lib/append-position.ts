...
export async function appendWithRetry(
  ...
  const { position } = await appendWithRetryReturning(
    readLastPosition,
    async (p) => {
      const { conflict } = await applyAt(p);
      return conflict
        ? { conflict: true }
        : { conflict: false, value: undefined };
    },
    maxAttempts,
  );
  return position;
...
/** applyAt 결과: 충돌이면 conflict, 아니면 호출부가 만든 값(예: 새 카드 id)을 동봉. */
export type ApplyOutcome<T> =
  | { conflict: true }
  | { conflict: false; value: T };

/**
 * {@link appendWithRetry} 의 값-반환 변형.
 *
 * 재시도 루프는 동일하지만, 충돌 없이 성공한 applyAt 이 만든 값(예: 직접 insert 한
 * 새 카드 id)을 확정 position 과 함께 돌려준다. createCard 처럼 append 결과의
 * 식별자가 필요한 호출부용 — appendWithRetry 는 이 함수의 얇은(값 버리는) 래퍼다.
 *
 * 충돌 진전 불변식은 appendWithRetry 와 동일: between(last,null) 은 항상 현재
 * 컬럼 최대값보다 strictly 크므로 비최대 행과는 충돌할 수 없고, 동시 append 끼리만
 * 충돌한다. 충돌한 쪽이 커밋되면 last 가 갱신되어 재계산값이 다시 strictly 커진다.
 *
 * @param readLastPosition 타깃 컬럼의 현재 마지막(최대) position. 빈 컬럼이면 null.
 * @param applyAt          계산된 position 으로 적용. unique 위반이면 `{conflict:true}`,
 *                         성공이면 `{conflict:false, value}`.
 * @returns 최종 확정된 position 과 성공한 applyAt 의 value.
 */
export async function appendWithRetryReturning<T>(
  readLastPosition: () => Promise<string | null>,
  applyAt: (position: string) => Promise<ApplyOutcome<T>>,
  maxAttempts = 5,
): Promise<{ position: string; value: T }> {
  for (let attempt = 1; ; attempt++) {
    const last = await readLastPosition();
    const position = between(last, null);
    const outcome = await applyAt(position);
    if (!outcome.conflict) return { position, value: outcome.value };
